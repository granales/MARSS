    ...
    class TRBehav(CyclicBehaviour):
        async def run(self):
            ...
            if tr_status_var == "on":
                ...
                await self.send(wh_msg_log)
                ca_tr_msg = await self.receive(timeout=wait_msg_time)  # wait for a message for 5 seconds
                if ca_tr_msg:
                    ca_data_df = pd.read_json(ca_tr_msg.body)
                    if ca_data_df.loc[0, 'action'] == "pre-book":
                        """Prepare reply to ca of availability"""
                        tr_msg_ca = opf.msg_to_sender(ca_tr_msg)
