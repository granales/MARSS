    ...
    class BRBehav(CyclicBehaviour):
        async def run(self):
            ...
            if br_status_var == "on":
                ...
                ca_br_msg = await self.receive(timeout=wait_msg_time)  # wait for a message for 5 seconds
                if ca_br_msg:
                    print(f'ca_br_msg: {ca_br_msg.body}')
                    ca_data_df = pd.read_json(ca_br_msg.body)
                    """Prepare reply"""
                    br_msg_ca = opf.msg_to_sender(ca_br_msg)
